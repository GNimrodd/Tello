    ...
    def get_frame_read(self):
        ...
        if self.background_frame_read is None:
            self.background_frame_read = BackgroundFrameRead(self, self.get_udp_video_address(), self.show_video,
                                                             self.name, f"tello_video_{int(time.time())}.h264").start()
...
class BackgroundFrameRead:
    ...
    def __init__(self, tello, address, show_video: bool = True, name: str = "", save_video=None):
    ...
    def update_frame(self):
        video_file = open(self.save_video, 'wb') if self.save_video else None
